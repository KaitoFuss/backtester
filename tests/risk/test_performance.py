...
def test_equity_curve_samples_portfolio_equity_per_bar() -> None:
    tracker = PerformanceTracker(portfolio=FakeEquitySource([1_000.0, 1_050.0]))
    ...
    assert tracker.equity_curve == [(_ts(0), 1_000.0), (_ts(1), 1_050.0)]


def test_metrics_with_flat_equity_are_zero() -> None:
    tracker = PerformanceTracker(portfolio=FakeEquitySource([1_000.0] * 5))
    for i in range(5):
...
def test_metrics_with_insufficient_history_are_zero() -> None:
    tracker = PerformanceTracker(portfolio=FakeEquitySource([1_000.0]))
...
def test_max_drawdown_reflects_peak_to_trough_decline() -> None:
    ...
    tracker = PerformanceTracker(portfolio=FakeEquitySource(values))
