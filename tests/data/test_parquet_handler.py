...
def _collect(handler: ParquetHandler) -> list[MarketEvent]:
...
def test_correct_number_of_events(data_dir: Path) -> None:
    events = _collect(ParquetHandler(data_dir))
    assert len(events) == 3
...
def test_bars_contain_expected_tickers(data_dir: Path) -> None:
    event = ParquetHandler(data_dir).get_next_bar()
...
def test_ticker_filtering(data_dir: Path) -> None:
    events = _collect(ParquetHandler(data_dir, tickers=["AAPL"]))
    for event in events:
...
def test_missing_ticker_in_file_skipped(tmp_path: Path) -> None:
    ...
    events = _collect(ParquetHandler(tmp_path, tickers=["AAPL", "MSFT"]))
