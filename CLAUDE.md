...
- `portfolio/`: `VolWeightedPortfolio` — opens long/short positions with conviction-per-risk weights (`score / trailing σ`, signed) normalized into the gross still available under `max_gross` (the leverage limit: gross exposure as a multiple of equity). Cash is tracked as an accounting balance, not a sizing constraint — the leverage cap is. Held positions are never resized (flat→open→flat band trading); a sign flip / sub-`exit_threshold` / zero score closes; an absent ticker is held. Falls back to score-proportional sizing (within the same gross budget) until every relevant ticker has `vol_window` returns.
...
- `risk/`: `PositionExitRiskManager` — flattens a position on stop-loss, take-profit, or max-holding-days breach; stateless, reading entry price/date/quantity from the `Portfolio` via `PortfolioView` rather than replaying the fill stream. `LeverageRiskManager` — caps gross exposure at `max_gross · equity`: passes reducing orders through, scales opening orders down to fit the remaining budget, and emits trim orders when mark-to-market drift pushes the held book past the ceiling. `ChainedRiskManager` threads several managers through the `Engine`'s single `RiskManager` slot (exits before the leverage cap). All satisfy the `RiskManager` protocol.
