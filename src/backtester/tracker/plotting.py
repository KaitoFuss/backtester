...
def draw_equity_curve(
    ...
    ax_equity.set_title("Equity Curve", color=_INK, fontsize=11, loc="left")
    ax_equity.set_ylabel("Portfolio value ($)", color=_MUTED, fontsize=8)
    if len(series) > 1:
        ax_equity.legend(frameon=False, labelcolor=_INK, fontsize=8)

    ax_drawdown.set_title("Drawdown", color=_INK, fontsize=11, loc="left")
    ax_drawdown.set_ylabel("Drawdown", color=_MUTED, fontsize=8)
